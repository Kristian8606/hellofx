...
public class Diagram implements Initializable {
    ...
    private void diagram() {
        ...
        series1.setName("Лихви");
        series2.setName("Д-ие каса");

        try {
            Connection con = DBConector.getConections();
            ResultSet rs = con.createStatement().executeQuery("select * from "+DBName+".tableCashbox;");
            while (rs.next()) {
                String a = rs.getString(2);
                double b = Double.parseDouble(rs.getString(7));
                double c = Double.parseDouble(rs.getString(6));
                series1.getData().add(new XYChart.Data<>(a, b));
                series2.getData().add(new XYChart.Data<>(a, (c < 0) ? 0 : c));
            }
        } catch (Exception ex) {
        ...
        */
        ...
        tooltipCreate(series1, "Лихви");
        tooltipCreate(series2,"Раздадени");
